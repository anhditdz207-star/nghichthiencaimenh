...
import { isBgMusicMuted, toggleBgMusicMute, onBgMusicMuteChange } from "../lib/bgMusic";

export default function MusicToggle() {
  const [muted, setMuted] = useState(isBgMusicMuted());

  useEffect(() => onBgMusicMuteChange(setMuted), []);

  return (
    <button
      onClick={() => setMuted(toggleBgMusicMute())}
      aria-label={muted ? "Bật nhạc nền" : "Tắt nhạc nền"}
      title={muted ? "Bật nhạc nền" : "Tắt nhạc nền"}
      className="fixed bottom-5 left-4 sm:left-6 z-40 w-11 h-11 rounded-full border border-gold-700/50 bg-ink-900/80 backdrop-blur flex items-center justify-center text-gold-400 hover:border-gold-500 transition-colors"
    >
      {muted ? (
